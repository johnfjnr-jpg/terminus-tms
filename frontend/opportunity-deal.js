...
import { scheduleReconciliation } from '/lib/milestone-schedule.js'
...
// ── THE VANILLA FORM IMPLEMENTING THE SEAM ───────────────────────────────
//
// The version machinery moved to opportunity-deal-versions.js in Round 3 D2c
// and now talks only to this object. While this file is still the live form,
// the vanilla implements the seam, so the version file runs against either
// form UNCHANGED. That is the whole point of the seam, and it is testable now
// rather than at the swap.
//
//   freezeCurrentState  dirty check, THEN saveDeal, THEN read - and it THROWS
//                       the refusal, where saveDeal returns falsy, because a
//                       refused save must not yield a frozen state.
//   recompute           returns the CURRENT PAYLOAD. The position recorded in
//                       D2c: saveVersion's refusal check needs the contractor
//                       base BEFORE any save, and this is the only member that
//                       can supply it without saving.
// ── A DELIBERATE DUPLICATE, AND THE ONLY ONE THE SPLIT CREATES ───────────
//
// `escapeSheet` moved with the version machinery, and `milestoneOptions` here
// still needs it. Three lines, pure, no state: a copy in each module is
// cheaper than a third shared module for one string escaper, and the drift
// risk of a pure formatter is close to nil.
//
// Recorded rather than hidden, because "close to nil" is not nil and the next
// person to change one should change both.
...
window.dealFormSeam = dealFormSeam
...
window.initOpportunityDealPanel = async function (opp) {
  ...
  recompute()
  // After the first render, so a slow list never delays the figures. The
  // version machinery is a separate module now and is HANDED what it needs
  // rather than sharing this file's scope.
