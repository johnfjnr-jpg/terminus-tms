...
document.getElementById('btn-back-contact-detail').addEventListener('click', () => {
  // ── THE VANILLA FALLBACK RETIRED, Round 8 Phase 2 ────────────────────
  //
  // This read `cdReturnView` too - the vanilla's own lexical, which app.js
  // could reach because classic scripts share one scope. `contact-detail.js`
  // is DELETED, so that binding can never exist and the branch was permanently
  // dead. It retires with its file.
  //
  // THE SEAM MEMBER ITSELF STAYS, and the brief's expectation that it would
  // retire is the one place Phase 2 departs from instruction. Measured by
  // deletion: removing `setContactReturnView` fails two ContactView tests. It
  // REPLACED the lexical read rather than depending on it - the React view
  // owns the answer and pushes it here - so it outlives the file it was built
  // to work around.
  //
  // `'leads'` is still the default, and it is the safe one: leads is where an
  // unqualified contact came from, and a contact with no status is unqualified.
  const fromSeam = typeof window.contactReturnView === 'function' ? window.contactReturnView() : null
  const view = fromSeam ?? 'leads'
