...
All 12 phases built and signed off: 0, **0A**, 1, 2, 3, 4, **5A**, **5B**, 6, 7,
8, 9. Checked with `grep -n "^## Phase\|^### Phase"` against this document per
rule 7, which returns 13 headings: the thirteenth is the Phase 5 preamble,
which is shared context for 5A and 5B rather than a phase to sign off. Phase
0A was added mid-round and 5 was split mid-round; both are included in the
count.
...
|---|---|---|
| 0. Investigate and report | All six items, with a regenerated `CURRENT_STATE.md` and a baselined suite | **Three of six came back against the brief's premise.** Item 2 reproduced a defect Round 8 could not, and found it **8 controls wide, not one**. Item 4's premise was refuted outright: neither contact dialogue navigates anywhere, and the real cause is an unfiltered `onAuthStateChange`. Item 3 split one suspected defect into two. My own first `state-dump` was taken while `test:db` was running and reported harness fixtures as configuration |
| 0A. One click reveals and opens | Scope addition, taken as its own phase. One shared helper across all four `open*Field` implementations, not the eight reported fields alone | The metric had to change with the mechanism: the mousedown count that **reproduced** the defect stays 0 under the fix and would have reported it as a failure. The decisive control was running the identical probe against pre-fix code, 0 of 8 against 8 of 8. `showPicker()` did **not** throw without user activation, which vindicated guarding on an explicit flag rather than on `navigator.userActivation` |
| 1. Test Bed name | Creation dialogue with the suffix pre-filled and pre-selected, plus an editable name on the Reference tab | The reversal is only safe because the **same phase** closes the gap Round 5 Phase 2 built around, so both halves shipped together. One computation path for the naming rule, shared by the create endpoint and a new read-only suggestion endpoint. My backward-compatibility check was wrong first time: Fastify rejects a JSON content-type with no body before any route code runs |
| 2. Reference tab layout | Summary in line with the name, Notes to the bottom with the two-most-recent default, panel reorder. Header 346px to 145px at 1920, 366px to 222px at 1240 | **The header notes digest had always shown the two OLDEST notes under the label "Latest notes"**, invisible because no live Test Bed had ever held more than one note. **Corrected my own Phase 0 claim**: there are zero duplicate element ids anywhere; three rows share a `data-key` value, which is not an identifier |
| 3. Labels, narrowing, Installation Environment | The confirmed label table, Customer Details narrowed, picklist with server-side validation | **The brief's premise for item 4 was empirically wrong**: `.ref-field-label` is a fixed 170px basis, so shortening label text frees no width at all. Three further things were needed, each measured. **The 3.2 migration was surveyed and deliberately not written**: the record the brief called live is soft deleted, and `record_revisions` is schema-level immutable. `renderTbSiteDetails` was ignoring its own field definitions, the **second** instance in one file |
| 4. New Contact dialogue | Mobile validation on both write paths; the navigation defect fixed at its real cause | **Region was already built, on a better key.** `regionForCountry()` has populated it from Country since the modal was built. City-to-Region declined: Newcastle spans three of the five regions. The auth guard is on **app state, not an event-name allowlist**, because event names vary by library version. A 500 I chased turned out to predate the phase and be RLS on my own fixture |
| 5A. Stale content | Three fetches parallelised, then a synchronous pending state. Criteria stale window **2034ms to 1083ms to zero** | Parallelising **required** a correctness fix first: `renderTbStageExitCriteria` had no token guard and was safe only by running last. The pending state then exposed that **`api()` had no `try/catch`**, so every caller's `!ok` branch was unreachable on a network failure - fixed in `api()` itself, reaching every screen |
| 5B. Action latency | Criterion tick **1162ms to 296ms**, approval tick **749ms to 416ms**, both server-confirmed rather than optimistic | Criterion writes **serialised**, because two concurrent PATCHes each merge into whatever revision they read and can drop the first tick. The approval path stopped bumping the tab load token, which is why a second approval used to invalidate the first. Also fixed a pre-existing `/api/records/null/exit-criteria` race |
| 6. Transition lands on the next tab | All seven transitions land on the stage just entered. Lifecycle **47 clicks**, 7 of them saved by this phase | Reported the comparison honestly: only 7 of the apparent 12-click saving belongs to this phase; the rest is a difference in counting. **Round 9's own figure is 59 stated and 58 itemised**. Four wait-condition faults in my driver, one of which reported the feature as not working on all seven transitions |
| 7. Closed shows the completed record | One read-only panel, nine documents grouped by stage in lifecycle order, and Phase 6's exception removed | Read-only is **structural**: the endpoint returns nothing a Confirm control could act on. **Both completed Test Beds hold zero `document_details` rows**, so the URL path had never been exercised and needed a purpose-built fixture. Found and fixed a real load-order race in the terminal-stage check. My CSS used two variables that do not exist in this stylesheet |
| 8. Sensors panel | Show Sensors toggle, one panel per sensor, identity and the existing not-linked state, nothing invented | The grid floor is **140px, measured**: an initial 118px clipped **35 of 42 names**, and identity is the only content these panels carry. Recorded as **the visible surface of a real gap**: the counts are unverifiable and two gates that read as physical checks are paperwork checks |
| 9. Regenerate and reconcile | `CURRENT_STATE.md` regenerated, every hunk reconciled, `stage_gate_rules` byte-identical at 54 total and 38 on `test_bed` | **The reconciliation found a defect in my own teardown**: 31 document children of soft-deleted probe Test Beds were still live, because every teardown selected by account while a document hangs off the Test Bed. **Second consecutive round in which this phase justified its own existence** |

### Four things only visible from the end of the round

**1. Eight wait-condition faults, in the tool built to measure a round about stale state.** Every one produced a confident, legible, wrong result. **Four were written in or after the phase that promoted the guard against them into `CLAUDE.md`**, by the person who had just written the guard. Several presented as product defects: one as a cross-record data-binding fault where a second Test Bed's approval rows appeared bound to the first record's id, which would have meant approvals recorded against the wrong record; another as a feature simply not working, the landing tab reading as the previous stage on all seven transitions. **None was a product defect.** The lesson is not that the rule is wrong but that **knowing it confers no ability to spot which conditions are stale-satisfiable** - the judgement is per-condition and per-selector. The form that survives is the counterfactual, now rule 7: state what the condition would look like if the action had not happened, and check it differs. That settles every one of the eight in a sentence, without knowing the feature.

**2. Three of Phase 0's six items came back against the brief's premise, and that is Phase 0 working.** Region was already built and keyed on Country, a strictly better field than the City the brief proposed. The Save navigation was an unfiltered auth listener, not either save path, and building the item as briefed would have changed working code and left the defect in place. Shortening labels frees no width, because the label column is a fixed basis. **A brief written from documents rather than from the code will contain wrong premises**, not through carelessness but because documents record intent and code records behaviour. This round's briefs are unusually good and still had three. **Phase 0 is what catches them, and its value is highest exactly when it disagrees with the document that commissioned it.**

**3. A path correct for every caller it has becomes wrong for the one about to be built, and the fault only goes live when something starts depending on the branch it was skipping.** `api()` had no `try/catch`, so every caller's `!ok` branch was unreachable on a network failure - harmless while a failed load merely left old content on screen, and a permanently stuck panel the moment a pending state existed. `renderTbStageExitCriteria` had no load-token guard and was safe only because it ran last, until the fetches were parallelised. Neither appeared as a regression and no test broke, because **nothing was broken until the new use arrived**. Now `CLAUDE.md` Architecture rule 8, with `complete-document` from Round 9 as the third instance.

**4. Rules restated in four and seven consecutive briefs had no permanent home, and repetition had created the appearance of establishment.** "Never verify on a fixed delay" had been restated in four briefs and appeared nowhere in `CLAUDE.md`; the layout test widths in seven; soft-delete-only teardown in four. **A brief restating a rule is evidence the rule matters and evidence it may have no home, and those look identical from inside a round.** All three are now promoted. The sweep that found them is cheap and worth repeating whenever a rule is restated rather than cited.

### Open, carried forward

1. **Sensor counts are unverifiable, and it is an argument for connecting Asset Management earlier than planned.** Nothing links a Test Bed to a device, so no part of the system checks that a Test Bed declaring 24 cameras ever had 24. `PROTOTYPE_SPECIFICATION.md` Section 2b records a **working** Device-to-Test-Bed link in the prototype that was never surfaced, and names the two gates it would give teeth to: Installation and Commissioning requiring N devices genuinely linked, Decommissioning requiring zero still linked. Both are document-only today, so two gates that read as physical checks are paperwork checks.
2. **Three probe auth users cannot be deleted.** `records.owner_id` is `not null references auth.users(id)` **with no `ON DELETE` clause**, so soft-deleted fixtures pin their owner permanently. They own 377 records between them, none live. Pinning to one probe user per round contained it from Phase 5B onward.
3. **`resolveOwnerId()` selects positionally**, taking `listUsers({page:1,perPage:1})[0]`, so any auth user created for any reason becomes the owner of every fixture the suite creates. Should resolve a **named** owner.
4. **`Fixtures.teardown()` only runs on the happy path**, sitting in an `after` hook, so a file aborting in `before` abandons everything it created. Recorded Round 9 Phase 7, unchanged.
5. **Harness `record_type` accumulation**: 116 distinct synthetic types and 1,415 soft-deleted rows. Known, unfixed, the same shape as the counter-accumulation finding whose remedy was never applied here.
6. **`document_details` has never carried a URL outside a purpose-built fixture.** Every completed Test Bed has nine approved documents and no recorded location. Not a defect; **directly relevant to Round 13**, where Customer Documents and Drive make the link the point. Worth deciding whether a URL should be required at document confirmation, which is a `stage_gate_rules` question.
7. **Phase 3.2's data migration was deliberately not written**, and the consequence is recorded rather than hidden: three soft-deleted records still hold legacy free-text Installation Environment values, and **a legacy value is silently discarded the next time that field is opened and saved**, because a `select` with no matching option falls back to blank. The validation guard protects unrelated saves and does not protect the field itself.
8. **Long contact names still clip in the narrowed Customer Details panel.** A 36-character name needs 281px and the select affords 188px. **Pre-existing** - it clipped at 272px before the narrowing too - so unchanged in kind, with less headroom.
9. **Two candidates from Round 9 remain costed and declined**: the migration-ledger parity invariant, and the `CURRENT_STATE.md` staleness gate, worth building as a merge or release check rather than per commit.
