 ...
 */
export function versionApprovalState(version, approvals, latestRevision) {
  ...
  const here = (approvals ?? []).filter(
    (a) => a.track === APPROVAL_TRACK && a.revision_number === at)
 ...
 */
export function lastApprovedVersion(versions, approvals, latestRevision) {
  const approvedOnes = (versions ?? [])
    .map((v) => ({ v, s: versionApprovalState(v, approvals, latestRevision) }))
