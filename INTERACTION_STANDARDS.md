...
**Two real, working examples now exist: New Lead, and Park (Section 4's own dialog example), which now has both halves of this pattern, not just the focus trap.** New Lead's backdrop-click-to-close (`frontend/app.js`, `newLeadDirty`, `openNewLeadModal`/`clearContactForm`, 2026-08-13) and Park's (`frontend/contact-detail.js`, `cdParkDirty`, `openCdParkForm`/`closeCdParkForm`, retrofitted 2026-08-13 from New Lead's implementation, not a second pattern - same `.btn-attention`/`.msg-warning` classes, same message text, same mechanism) both refuse to close their modal on an accidental click outside it while a field has genuinely changed since opening, highlighting Save and showing "You have unsaved changes, save or cancel." instead of silently discarding the draft. Both are deliberately narrow, the same way Park's focus trap is a contained slice of Section 4 rather than the whole dialog system: they guard backdrop-click only, Escape/Cancel/the close X all remain explicit, immediate-discard actions, unchanged in both, and neither has any connection to real page navigation at all - neither is the system-wide dirty-state registry this section specifies. They're a working proof that the underlying idea, don't silently discard real unsaved input, holds up in two small, real cases, for whenever the full system-wide version gets built.
