...
**A second real, working example now exists, alongside Park's focus trap (Section 4):** New Lead's backdrop-click-to-close (`frontend/app.js`, `newLeadDirty`, `openNewLeadModal`/`clearContactForm`, 2026-08-13) refuses to close the modal on an accidental click outside it while any field has genuinely changed since opening, highlighting Save and showing a message instead of silently discarding the draft. It is deliberately narrow, the same way Park's focus trap is a contained slice of Section 4 rather than the whole dialog system: it guards backdrop-click only, Escape/Cancel/the close X all remain explicit, immediate-discard actions, unchanged, and it has no connection to real page navigation at all, it is not the system-wide dirty-state registry this section specifies. It's a working proof that the underlying idea, don't silently discard real unsaved input, holds up in one small, real case, for whenever the full system-wide version gets built.
