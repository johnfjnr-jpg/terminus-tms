...
const SP = '/private/tmp/claude-501/-Users-johnfryatt-terminus-tms/0a84a38b-2e9e-4092-bcb4-85f9dfb560c7/scratchpad/cal-cmode'
...
const INJECTIONS = [
  { f: FILES[0], from: "{qualified ? 'Contact details' : 'Lead details'}", to: "{'Lead details'}",
    what: 'R1 the title stops being mode-aware (always lead)' },
  { f: FILES[0], from: "{qualified ? 'Contact details' : 'Lead details'}", to: "{'Contact details'}",
    what: 'R1 the title stops being mode-aware (always contact)' },
  { f: FILES[0], from: '{status && !qualified', to: '{status',
    what: 'R2 the chip comes back on a contact' },
  { f: FILES[1], from: '{!qualified\n        ? <button type="button" className="btn-ghost"', to: '{true\n        ? <button type="button" className="btn-ghost"',
    what: 'R3 Nurture comes back on a contact' },
  { f: FILES[1], from: 'className="btn-primary"', to: 'className=""',
    what: 'the Qualify treatment is dropped again' },
  { f: FILES[1], from: 'onClick={() => { setMenuOpen(false); onCreate(\'opportunity\') }}', to: 'onClick={() => { setMenuOpen(false) }}',
    what: 'R4 choosing Opportunity stops reaching the shell' },
  { f: FILES[1], from: "if (e.key === 'Escape') setMenuOpen(false)", to: "if (e.key === 'NeverPressed') setMenuOpen(false)",
    what: 'R4 the menu stops closing on Escape' },
]
