...
try {
  ...
  // INDEPENDENT STATE? Type into one and read the other.
  await page.click(`[data-testid="lead-fix-summary-${lead.id}"]`)
  await page.type(`[data-testid="lead-fix-summary-${lead.id}"]`, 'typed into the surface')
  const drift = await page.evaluate((x) => ({
    surface: document.querySelector(`[data-testid="lead-fix-summary-${x}"]`)?.value,
    card: document.querySelector(`[data-testid="lead-summary-input-${x}"]`)?.value,
  }), lead.id)
  console.log(`  after typing into the SURFACE: surface="${drift.surface}" card="${drift.card}"`)
  console.log(`  -> independent copies: ${drift.surface !== drift.card}`)
  ...
  console.log(`  with R2 applied the Summary editor goes, leaving ${only.inputs - 1} inputs and the star`)
