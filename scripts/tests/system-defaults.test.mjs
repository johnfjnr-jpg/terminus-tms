...
test('the defaults are written at CREATION and read NOWHERE ELSE', () => {
  // THE PROPERTY THE WHOLE DESIGN RESTS ON. An initial value lives in the
  // record; a fallback lives in the calculation. A second caller reached from a
  // render, a recompute or a save would turn this back into a fallback, and
  // nothing else in the suite would notice because the numbers would look
  // right.
  //
  // Measured across every route and lib rather than asserted.
  const callers = []
  const walk = (dir) => {
  ...
  walk('src/')
  callers.sort()
  assert.deepEqual(callers, ['src/lib/system-defaults.js', 'src/routes/contacts.js', 'src/routes/test-beds.js'],
    'readSystemDefaults is called somewhere new; defaults must be read only at creation')
