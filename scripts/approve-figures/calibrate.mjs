...
const SNAP = `${ROOT}/.verify/approve-figures/snapshots`
...
const INJECTIONS = [
  { id: 'K1 pricing the snapshot at all',
    file: 'src/lib/approval-page.js',
    find: "  const priced = version?.inputs ?? payload;",
    put: "  const priced = payload;",
    expect: 'R1' },
  { id: 'K2 the FROZEN rates',
    file: 'src/lib/approval-page.js',
    find: "  const pricedRates = version ? (version.rates?.rates ?? {}) : (catalog.rates ?? {});",
    put: "  const pricedRates = catalog.rates ?? {};",
    expect: 'R3' },
  { id: 'K3 the target reading the priced thing',
    file: 'src/lib/approval-page.js',
    find: "  const target = buildTarget(priced, result, {",
    put: "  const target = buildTarget(payload, result, {",
    expect: 'R8' },
  { id: 'K4 the route passing rates',
    file: 'src/routes/deal-sheet-versions.js',
    find: "      catalog: { rates: catalog.rates, batches: catalog.batches, missing: catalog.missing, asOf: catalog.asOf },",
    put: "      catalog: { batches: catalog.batches, missing: catalog.missing, asOf: catalog.asOf },",
    expect: 'R11' },
]
...
const run = () => {
  const out = `${ROOT}/.verify/approve-figures/cal-run.txt`
  try {
    execSync('npm test', { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'] })
