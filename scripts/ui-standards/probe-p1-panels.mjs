// UI STANDARDS - LEADS, Phase 1: the SAME inventory, after the retrofit.
//
// Copied from the Phase 0 probe with its artefact paths re-pointed first
// (Verification 44's lineage clause). The claims and the measuring function
// are UNCHANGED - the same instrument, before and after, which is the only
...
//
// Artefacts are `uis-p1-*` (Verification 44's lineage clause).
import { loadPuppeteer } from '../lib/puppeteer.mjs'
const puppeteer = await loadPuppeteer('probe-p1-panels.mjs')
...
try {
  ...
  await page.evaluateOnNewDocument(() => {
    window.__uisMeasure = (panelSel, opts) => {
      ...
      const saveEl = q(opts.save)
      const save = R(saveEl)
      if (save) save.inFooter = !!saveEl.closest('.form-actions')
  ...
  const classify = (m) => {
    ...
    // A MODAL FOOTER IS ITS OWN SHAPE, ruled by John before this measurement
    // and not a variety of "below the field". The classifier asked the panel
    // question of every surface, so it reported the two dialogues as S1
    // violations for obeying the convention they were told to obey.
    if (m.save.inFooter) return 'FOOTER (modal, ruled)'
  ...
  report('SUMMARY', await page.evaluate((x) => window.__uisMeasure(`[data-testid="lead-summary-${x}"]`, {
    header: '.panel-head', title: '[data-panel-title]', secondary: null,
    field: `[data-testid="lead-summary-input-${x}"]`, save: `[data-testid="lead-summary-save-${x}"]`,
    discard: `[data-testid="lead-summary-discard-${x}"]` }), ID))

  report('NOTES', await page.evaluate((x) => window.__uisMeasure(`[data-testid="lead-notes-${x}"]`, {
    header: '[data-testid="cd-notes-header-row"]', title: '[data-panel-title]',
    secondary: '.panel-secondary', field: '[data-testid="cd-new-note-input"]',
  ...
  report('NOTES (editor open)', await page.evaluate((x) => window.__uisMeasure(`[data-testid="lead-notes-${x}"]`, {
    ...
    secondary: '.panel-secondary', field: '[data-testid="cd-new-note-input"]',
    save: '[data-testid="cd-add-note-btn"]', discard: '[data-testid="cd-note-discard"]' }), ID))
  await shot(ID, 'uis-p1-panels-1920.png')
  ...
  await page.screenshot({ path: `${OUT}uis-p1-address-1920.png` })
  ...
  await page.screenshot({ path: `${OUT}uis-p1-nurture-1920.png` })
  ...
  await shot(picker.id, 'uis-p1-picker-1920.png')

  // ── S1 AND S3 AT THREE WIDTHS ────────────────────────────────────────
  // The standing verification. S1's right-alignment and S3's field
  // alignment are geometry, and geometry is the half a unit test cannot
  // answer. Asserted as RELATIONSHIPS: save-vs-panel-right and
  // field-vs-header-left, never a property of one element.
  console.log('\n=== S1 and S3 at three widths ===')
  for (const w of [1240, 1920, 3440]) {
    ...
    await go(ID)
    const rows = []
    for (const [label, sel, save] of [
      ['SUMMARY', `[data-testid="lead-summary-${ID}"]`, `[data-testid="lead-summary-save-${ID}"]`],
      ['NOTES', `[data-testid="lead-notes-${ID}"]`, '[data-testid="cd-add-note-btn"]'],
    ]) {
      const m = await page.evaluate((s, sv) => window.__uisMeasure(s, {
        header: '.panel-head', title: '[data-panel-title]', secondary: null,
        field: 'textarea', save: sv, discard: null }), sel, save)
      if (m.missing) { rows.push(`${label}: absent`); continue }
      const rightGap = Math.round(m.panel.right - m.save.right)
      const onLine = Math.abs(m.save.top - m.header.top) < 10
      const leftGap = m.field ? Math.round(m.field.left - m.header.left) : null
      rows.push(`${label.padEnd(8)} on the header line ${String(onLine).padEnd(5)}`
        + ` right gap ${String(rightGap).padStart(3)}px  field-vs-header left ${leftGap}px`)
      if (!onLine) FAILED.push(`${w}: ${label} save is not on the header line`)
      if (rightGap > 2) FAILED.push(`${w}: ${label} save is ${rightGap}px off the right edge`)
      if (leftGap !== null && Math.abs(leftGap) > 2) FAILED.push(`${w}: ${label} field is ${leftGap}px off the header`)
    }
    console.log(`  ${w}px`)
    for (const r of rows) console.log(`    ${r}`)
    await shot(ID, `uis-p1-panels-${w}.png`)
  }
  console.log(FAILED.length ? `\n  WIDTH FAILURES:\n    ${FAILED.join('\n    ')}` : '\n  all three widths conform')

  console.log('\n=== SUMMARY OF THE INVENTORY ===')
  const placements = [...new Set(INVENTORY.map((i) => i.s1))]
  // The headline the standard is about: among the surfaces S1 GOVERNS - the
  // inline panels, excluding the ruled modal shape, the frozen panel and the
  // record bar - how many placements are there?
  const governed = INVENTORY.filter((i) => !/modal|FROZEN|CARD ACTIONS/.test(i.name))
  const governedPlacements = [...new Set(governed.map((i) => i.s1))]
  console.log(`  AMONG THE PANELS S1 GOVERNS: ${governedPlacements.length} placement(s)`
    + ` - ${governedPlacements.join(', ')}`)
  console.log(`    (${governed.map((i) => i.name).join(', ')})`)
  console.log(`  across every surface, including the ruled shapes: ${placements.length}`)
