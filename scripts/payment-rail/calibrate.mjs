...
const SNAP = `${ROOT}/.verify/payment-rail/snapshots`
...
const INJECTIONS = [
  { id: 'Q1 the radios living in the rail',
    file: 'frontend-react/src/deal/section5.tsx',
    find: '            <div className="pt-rail" id="deal-payment-rail" data-testid="deal-payment-rail">',
    put: '            <div className="pt-rail" id="deal-payment-railX" data-testid="deal-payment-rail">',
    expect: 'P3' },
  { id: 'Q2 the label and its explanation being two things',
    file: 'frontend-react/src/deal/section5.tsx',
    find: "  { value: 'single', label: 'Single phase', note: 'recovery over full term' },",
    put: "  { value: 'single', label: 'Single phase (recovery over full term)' },",
    expect: 'P4' },
  { id: 'Q3 the yearly table being rendered once',
    file: 'frontend-react/src/deal/section5.tsx',
    find: '              {opexOn ? null : <div id="deal-capex-year-slot">{yearSchedule}</div>}',
    put: '              <div id="deal-capex-year-slot">{yearSchedule}</div>',
    expect: 'P7' },
  { id: 'Q4 the top alignment',
    file: 'frontend/style.css',
    find: "#deal-capex-year-slot > * > .label:first-child { margin-top: 0; }",
    put: "",
    expect: 'the money starts at the SAME height in both modes' },
]
...
const run = () => {
  const out = `${ROOT}/.verify/payment-rail/cal-run.txt`
  try {
    let text = ''
    text += String(execSync('npm run test:react', { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'] }))
    text += String(execSync('node --env-file=.env scripts/payment-rail/probe-live.mjs',
...
try {
  for (const inj of INJECTIONS) {
    ...
    const named = text.includes(`FAIL  ${inj.expect}`)
      || new RegExp(`\u00d7 ${inj.expect}[:b ]`).test(text)
