...
- **Tab/Enter field navigation and unsaved-changes-on-navigate warnings**: system-wide, not screen-specific. Sized comparable to the Contact detail view itself, not a quick add-on - no `<form>` elements exist anywhere in the app today (Enter currently does nothing in any input), and unsaved-changes detection needs a generic dirty-state registry wired into every editable screen and into `navigate()` centrally. Not built. **Specification written** (`INTERACTION_STANDARDS.md`, sourced from the GOV.UK Design System's error-summary pattern and the WAI-ARIA Authoring Practices Guide), so this item has a concrete target to build against when it's picked up, not just a size estimate.
