...
- **TRACK COUNT AND ALIGNMENT AT WIDE VIEWPORTS. Raised 2026-08-29, Round 39.
  ...
  **At 3440 the card grid allocates more 460px tracks than there are cards, so
  three cards sit in the left 1380px and 2060px is empty.** The question is track
  count and alignment, not width.

  **The Deal Summary is already fluid and correct**, tracking the panel at every
  width: 876 at 1240, 1556 at 1920, 3076 at 3440. **The 396px field note is 62ch
  and must not change.**

  The Result list and the Versions card are fixed at 460px at all three widths.
  **Recorded as observed, not as a defect**: widening a list of short
  label-and-figure rows helps nobody.

  **HOW THIS ITEM WAS WRONG, and both halves are recorded because both parties
  own one.** I reported "content stops at roughly 1150px, the left third",
  **read off a screenshot rather than measured**. The business then framed the
  item as which regions escape the cap, and named the Deal Summary as the region
  that should escape it. **Measured, the Deal Summary is the one thing already
  working.**

  **Their own note, and it is `CLAUDE.md` rule 29's forward half turned on
  themselves:** when an instruction rests on a figure read off an image, say so,
  so the next round knows what to re-measure. Recorded here rather than only in
  the conversation, because the item would otherwise have been actioned against
  a number nobody could trace.
